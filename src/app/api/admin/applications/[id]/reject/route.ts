...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  // Fetch full application before rejecting so we can send evaluation email
  ...
  const { error } = await supabase.rpc('reject_application', { app_id: id });
  ...
  if (application?.founder_email) {
    ...
    const emailData = applicationStatusEmail({
      ...
      status: 'rejected',
      locale,
      dashboardUrl: `https://zeroen.dev/${locale}/dashboard/apply`,
      evaluation: {
